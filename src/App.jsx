...
const WEEKLY_PRAYERS = {
  ...
  2: { morning: { title: "St. Teresa of Avila Prayer", text: "Grant that in all things, great and small, today and all the days of my life, I may do whatever You require of me. Help me respond to the slightest prompting of Your Grace. Amen." }, afternoon: { title: "Glory Be", text: "Glory be to the Father, and to the Son, and to the Holy Spirit. As it was in the beginning, is now, and ever shall be. Amen." }},
  3: { morning: { title: "Memorare", text: "Remember, O most gracious Virgin Mary, that never was it known that anyone who fled to thy protection, implored thy help, or sought thy intercession was left unaided. Amen." }, afternoon: { title: "Hail Mary", text: "Hail Mary, full of grace, the Lord is with thee. Blessed art thou among women and blessed is the fruit of thy womb, Jesus. Amen." }},
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (day >= 1 && day <= 5) {
      ...
      setDailyPrayer({ ...(isMorning ? todayData.morning : todayData.afternoon), period: isMorning ? "Morning" : "Afternoon" });
    } else {
      setDailyPrayer({ title: "Lancer Blessing", text: "May the Lord bless the Salpointe community this weekend. Our Lady of Mount Carmel, pray for us.", period: "Weekend" });
    }
    return () => unsubscribe();
  }, []);
    ...
    <div className="h-screen w-screen flex flex-col bg-slate-950 text-slate-200 font-sans overflow-hidden">
      ...
      <main className="relative z-10 flex-grow flex overflow-hidden p-8 gap-8">
          ...
          <div>
            ...
            {dailyPrayer && (
              <div className="p-10 rounded-2xl bg-white/5 border border-white/10 shadow-2xl animate-fade-in">
                ...
                <p className="text-slate-200 font-serif text-2xl leading-relaxed first-letter:text-5xl first-letter:text-[#C5B358] first-letter:mr-2">{dailyPrayer.text}</p>
              </div>
            )}
          </div>
        ...
        <div className="w-1/2 flex flex-col overflow-hidden bg-black/20 rounded-2xl border border-white/5">
          <div className="p-6 border-b border-white/5 flex justify-between items-center"><h3 className="text-xs uppercase tracking-widest text-slate-500 font-bold">Community Intentions</h3><span className="w-2 h-2 rounded-full bg-[#C5B358] animate-pulse"></span></div>
      ...
      <div className="fixed bottom-10 right-10 z-50">
        <button onClick={() => user ? setIsModalOpen(true) : handleAuth()} className="w-20 h-20 rounded-full bg-[#2a0a0a] border-2 border-[#681818]/50 flex items-center justify-center shadow-2xl hover:scale-110 transition-transform"><Flame className="w-10 h-10 text-[#C5B358] candle-glow" /></button>
      </div>
