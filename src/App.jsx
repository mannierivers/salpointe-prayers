...
import { Flame, X, Info, Loader2, BookOpen, Clock, User, LogOut, LayoutDashboard, Trash2, Heart, ChevronLeft, ChevronRight, ArrowLeft, ArrowRight, RotateCcw } from 'lucide-react';
...
// Array mapping for linear navigation (0-9)
...
const WEEKLY_PRAYERS = {
  1: { 
    ...
    afternoon: { title: "Our Father", text: "Our Father, who art in heaven, hallowed be thy name; thy kingdom come, thy will be done, on earth as it is in heaven. Give us this day our daily bread and forgive us our trespasses, as we forgive those who trespass against us. Amen." } 
  },
  2: { 
  ...
  3: { 
    morning: { title: "Memorare", text: "Remember, O most gracious Virgin Mary, that never was it known that anyone who fled to thy protection, implored thy help, or sought thy intercession was left unaided. Inspired with this confidence, we turn to thee, O Virgin of virgins, our Mother. Amen." }, 
...
export default function App() {
  ...
  const navigatePrayer = (direction) => {
    if (direction === 'next') {
      setCurrentPrayerIndex((prev) => (prev === 9 ? 0 : prev + 1));
    } else {
      setCurrentPrayerIndex((prev) => (prev === 0 ? 9 : prev - 1));
    }
  };

  const resetToToday = () => {
    setCurrentPrayerIndex(actualNowIndex);
    showToast("Returned to today's prayer");
  };
  ...
  const activePrayer = WEEKLY_PRAYERS[activeSlot.day][activeSlot.period];
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-950 text-slate-200 font-sans overflow-hidden select-none">
      ...
      <main className="relative z-10 flex-grow flex overflow-hidden">
        ...
        <div className={`flex-grow flex flex-col items-center justify-center p-8 transition-all duration-700 ${isIntentionsExpanded ? 'pr-8' : 'pr-16'}`}>
          <div className="max-w-4xl w-full relative">
            ...
            <div className="group relative">
              <button onClick={() => navigatePrayer('prev')} className="absolute -left-16 top-1/2 -translate-y-1/2 p-4 text-slate-600 hover:text-[#C5B358] transition-all opacity-0 group-hover:opacity-100 hidden md:block">
                <ChevronLeft className="w-10 h-10" />
              </button>
              <button onClick={() => navigatePrayer('next')} className="absolute -right-16 top-1/2 -translate-y-1/2 p-4 text-slate-600 hover:text-[#C5B358] transition-all opacity-0 group-hover:opacity-100 hidden md:block">
                <ChevronRight className="w-10 h-10" />
              </button>

              <div className="p-12 md:p-16 rounded-3xl bg-white/[0.03] border border-white/10 shadow-2xl backdrop-blur-md animate-fade-in relative">
                <div className="flex flex-col items-center gap-4 mb-8">
                  <div className={`flex items-center gap-2 px-4 py-1 rounded-full border ${isViewingCurrent ? 'border-[#C5B358]/40 text-[#C5B358]' : 'border-white/10 text-slate-500'} uppercase tracking-[0.3em] text-[10px] font-bold`}>
                    ...
                    {activeSlot.label} {isViewingCurrent && "(Current)"}
                  </div>
                  
                  {!isViewingCurrent && (
                    <button 
                      onClick={resetToToday}
                      className="flex items-center gap-2 text-[#C5B358] hover:text-white transition-colors text-[9px] uppercase tracking-widest bg-[#C5B358]/10 px-3 py-1 rounded-full border border-[#C5B358]/20"
                    >
                ...
                <h2 className="text-4xl text-[#e8dcb5] font-serif mb-8 italic text-center leading-tight">{activePrayer.title}</h2>
                <div className="overflow-y-auto no-scrollbar max-h-[40vh] px-4">
        ...
        {/* SIDEBAR */}
        <div className={`transition-width border-l border-white/5 bg-black/40 backdrop-blur-xl flex flex-col overflow-hidden relative ${isIntentionsExpanded ? 'w-1/3' : 'w-16'}`}>
          <button onClick={() => setIsIntentionsExpanded(!isIntentionsExpanded)} className="absolute left-0 top-1/2 -translate-y-1/2 z-30 bg-white/5 hover:bg-white/10 p-1 rounded-r-md border border-l-0 border-white/10">
          ...
          <div className="p-6 border-b border-white/5 flex items-center gap-4">
            ...
            {isIntentionsExpanded && <h3 className="text-xs uppercase tracking-widest text-slate-500 font-bold whitespace-nowrap">Community Intentions</h3>}
          </div>
          <div className={`flex-grow overflow-y-auto p-6 no-scrollbar space-y-10 ${!isIntentionsExpanded && 'opacity-0'}`}>
            {loading ? <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin text-slate-700" /></div> : 
              prayers.map((p, i) => (
                <div key={p.id} className="flex gap-6 animate-fade-in group">
                  ...
                  <div className="flex flex-col flex-grow">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      {isAdmin && <button onClick={() => handleDelete(p.id)} className="text-red-900/40 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>}
                    </div>
                  </div>
                </div>
              ))
          ...
          {!isIntentionsExpanded && (
            <div className="absolute inset-0 top-20 flex flex-col items-center pointer-events-none opacity-40">
               <span className="[writing-mode:vertical-lr] uppercase tracking-[0.5em] text-[10px] text-slate-500 font-bold rotate-180">Intentions</span>
            </div>
          )}
        </div>
      </main>
      ...
      {/* Intro & Modal & Toast remain unchanged ... */}
      {isIntroOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-6 animate-soft-motion">
          <div className="max-w-2xl w-full bg-[#1e1e1e]/60 border border-[#C5B358]/20 rounded-3xl shadow-2xl p-10 text-center">
            ...
            <div className="space-y-6">
              <p className="text-[#e8dcb5] font-serif text-2xl leading-relaxed italic">"Please help strengthen our communication with God by focusing your mind, heart, and spirit in prayer for the intentions of the Salpointe community."</p>
              <p className="text-[#C5B358] font-serif text-2xl font-semibold italic">"Our Lady of Mt. Carmel, pray for us."</p>
            </div>
            <button onClick={() => setIsIntroOpen(false)} className="mt-12 px-12 py-4 bg-[#681818]/20 border border-[#681818]/40 text-[#e8dcb5] rounded-full font-serif text-xl hover:bg-[#681818]/40 transition-all tracking-wide">Enter the Chapel</button>
          ...
          <div className="bg-slate-900 border border-white/10 p-12 rounded-3xl w-full max-w-2xl shadow-2xl">
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              <div className="flex gap-4">
                ...
                <button onClick={handleSubmit} disabled={isSubmitting || !newPrayer.trim()} className="bg-[#681818] text-[#e8dcb5] px-12 py-3 rounded-full text-xl font-serif transition-all">Amen</button>
              </div>
            </div>
          </div>
        </div>
