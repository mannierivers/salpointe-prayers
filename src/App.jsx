...
// --- Weekly Prayer Data (Deacon Scott Pickett's Curated Cycle) ---
const WEEKLY_PRAYERS = {
  1: { // Monday
    morning: { title: "Morning Offering", text: "O Jesus, through the Immaculate Heart of Mary, I offer you my prayers, works, joys, and sufferings of this day in union with the holy sacrifice of the Mass throughout the world. I offer them for all the intentions of your sacred heart. Amen." },
  ...
  2: { // Tuesday
    morning: { title: "St. Teresa of Avila Prayer", text: "Grant that in all things, great and small, today and all the days of my life, I may do whatever You require of me. Help me respond to the slightest prompting of Your Grace, so that I may be Your trustworthy instrument. Amen." },
  ...
  3: { // Wednesday
    morning: { title: "Memorare", text: "Remember, O most gracious Virgin Mary, that never was it known that anyone who fled to thy protection, implored thy help, or sought thy intercession was left unaided. Inspired with this confidence, we turn to thee. Amen." },
    afternoon: { title: "Hail Mary", text: "Hail Mary, full of grace, the Lord is with thee. Blessed art thou among women and blessed is the fruit of thy womb, Jesus. Holy Mary, mother of God, pray for us sinners. Amen." }
  },
  4: { // Thursday
  ...
  5: { // Friday
    ...
    afternoon: { title: "Anima Christi", text: "Soul of Christ, make me holy. Body of Christ, save me. Blood of Christ, fill me with love. Water from Christ’s side, wash me. Passion of Christ, strengthen me. Good Jesus, hear me. Amen." }
  }
};
...
const styles = `
  ...
  html, body, #root { 
    height: 100vh; width: 100vw; overflow: hidden; margin: 0; background-color: #020617;
  }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      if (u && u.email.endsWith('@salpointe.org')) {
        setUser(u);
      } else if (u) {
        signOut(auth);
        showToast("Access restricted to Salpointe emails.");
      } else {
        setUser(null);
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'prayers'), {
        text: newPrayer.trim(),
        uid: user.uid,
        userName: user.displayName || user.email.split('@')[0], // Fallback if name is empty
        userEmail: user.email,
        timestamp: serverTimestamp()
      });
      setNewPrayer(''); setIsModalOpen(false); showToast("Intention shared.");
    } catch (e) {
      showToast("Error: Check your connection or login.");
    } finally {
    ...
    <div className="h-screen w-screen flex flex-col bg-slate-950 text-slate-200 font-sans overflow-hidden">
      ...
      <nav className="relative z-20 w-full p-6 flex justify-between items-center border-b border-white/5 bg-slate-950/50 backdrop-blur-md">
        <div className="flex items-center gap-8">
          ...
          <a href="https://teacher-agenda.vercel.app" target="_blank" rel="noreferrer" className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-slate-400 hover:text-[#e8dcb5] text-[10px] uppercase tracking-widest font-semibold">
            <LayoutDashboard className="w-3.5 h-3.5" /> Teacher Agenda
          </a>
        </div>
        
        <div className="flex items-center gap-6">
          {user && <span className="text-[#e8dcb5]/60 text-xs font-serif italic">{user.displayName}</span>}
          <button onClick={handleAuth} className="text-[10px] tracking-[0.2em] uppercase text-slate-500 hover:text-[#e8dcb5]">
      ...
      <main className="relative z-10 flex-grow flex overflow-hidden p-8 gap-8">
        ...
        <div className="w-1/2 flex flex-col overflow-hidden bg-black/20 rounded-2xl border border-white/5">
          ...
          <div className="flex-grow overflow-y-auto p-8 no-scrollbar space-y-10">
            {loading ? <div className="h-full flex items-center justify-center opacity-30"><Loader2 className="w-8 h-8 animate-spin" /></div> : 
              prayers.map((p, i) => (
      ...
      <div className="fixed bottom-10 right-10 z-50">
        <button onClick={() => user ? setIsModalOpen(true) : showToast("Sign in to light a candle.")} className="w-20 h-20 rounded-full bg-[#2a0a0a] border-2 border-[#681818]/50 flex items-center justify-center shadow-xl hover:scale-110 transition-transform">
          ...
          <div className="bg-slate-900 border border-white/10 p-12 rounded-3xl w-full max-w-2xl shadow-2xl animate-fade-in">
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              <div className="flex gap-4">
                ...
                <button onClick={handleSubmit} disabled={isSubmitting || !newPrayer.trim()} className="bg-[#681818] text-[#e8dcb5] px-12 py-3 rounded-full text-xl font-serif">
                  {isSubmitting ? <Loader2 className="animate-spin" /> : "Amen"}
                </button>
              </div>
            </div>
          </div>
